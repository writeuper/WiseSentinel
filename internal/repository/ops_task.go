...
func (r *OpsTaskRepo) Create(ctx context.Context, task *OpsTask) error {
	...
	if strings.TrimSpace(task.TaskContractJSON) != "" {
		data["task_contract_json"] = task.TaskContractJSON
	}
	_, err := g.DB().Insert(ctx, "ws_ops_task", data)
	return err
}
...
func (r *OpsTaskRepo) Get(ctx context.Context, tenantID, taskID string) (*OpsTask, error) {
	var row struct {
		...
		TaskContractJSON string     `json:"task_contract_json"`
		CompletionJSON   string     `json:"completion_json"`
	...
	return &OpsTask{
		...
		TaskContractJSON: row.TaskContractJSON,
		CompletionJSON:   row.CompletionJSON,
...
// Cancel transitions a runnable or active task into its terminal cancelled
// state. Clearing the execution token fences a worker that was already
// running; its context watcher will then stop model and tool work.
func (r *OpsTaskRepo) Cancel(ctx context.Context, tenantID, taskID string) (bool, error) {
	now := time.Now()
	result, err := g.DB().Model("ws_ops_task").Ctx(ctx).Where("tenant_id", tenantID).Where("task_id", taskID).
		Where("status IN(?)", []string{"pending", "retrying", "running", "awaiting_approval"}).
		Data(g.Map{"status": "cancelled", "result": "task cancelled by user", "last_error": "task cancelled by user", "finished_at": now, "execution_token": "", "next_attempt_at": nil}).Update()
	if err != nil {
		return false, err
	}
	rows, err := result.RowsAffected()
...
func (r *OpsTaskRepo) FinishIfOwned(ctx context.Context, tenantID, taskID, executionToken, status, resultText, detailJSON string) (bool, error) {
	...
	if status == "failed" || status == "timeout" || status == "incomplete" || status == "cancelled" {
...
// SetCompletionIfOwned stores the bounded completion decision before the
// terminal transition. It is fenced by the same execution token as the final
// write, so a stale worker cannot overwrite the current attempt's evidence.
func (r *OpsTaskRepo) SetCompletionIfOwned(ctx context.Context, tenantID, taskID, executionToken, completionJSON string) error {
	if strings.TrimSpace(completionJSON) == "" {
		return nil
	}
	_, err := g.DB().Model("ws_ops_task").Ctx(ctx).
		Where("tenant_id", tenantID).Where("task_id", taskID).
		Where("status", "running").Where("execution_token", executionToken).
		Data(g.Map{"completion_json": redact.TelemetryProjection(completionJSON)}).Update()
	return err
}
